...
class MIFGSMTS():
    def __init__(self, model, loss_fn, device):
        """
        Методика состязательной атаки методом momentum iterative FSGM на временной ряд. 
    ...
    def attack(self, X, y, eps=0.05, num_steps=10, alpha=None, decay=1.0, clip_min=None, clip_max=None, targeted = False, num_elem = None):
        ...
        # Проверка корректности выбранной величины decay
        if decay < 0:
            raise ValueError(
                "decay должен быть больше или равен 0, decay {} не допустим".format(decay)
            )
