...
class NIFGSMTS():
    def __init__(self, model, loss_fn, device):
        """
        Методика состязательной атаки методом Nesterov iterative FSGM на временной ряд. 
    ...
    def attack(self, X, y, eps=0.05, num_steps=10, alpha=None, decay=1.0, clip_min=None, clip_max=None, targeted = False, num_elem = None):
        """
        ...
        параметр: decay - коэффициент влияния момента
        ...
        momentum = torch.zeros_like(X, device=X.device)
        ...
        for _ in range (num_steps):
            ...
            grad = decay * momentum + grad / grad_norm
            momentum = grad
            X_adv = X_adv.detach() + alpha * grad.sign()
            delta = torch.clamp(X_adv - X, -eps, eps)

            # Создание атакованного экземпляра временного ряда
            X_adv = torch.clamp(X + delta, X - eps, X + eps).detach()
